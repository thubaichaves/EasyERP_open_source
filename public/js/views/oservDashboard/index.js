define([
    ...
    var View = Backbone.View.extend({
        ...
        contentType  : CONSTANTS.DASHBOARD_OSERV,
        ...
        initialize: function (options) {
            ...
            filter = this.filter = options.filter || custom.retriveFromCash('DashOserv.filter');
            ...
            dashCollection = this.dashCollection = options.collection || custom.retriveFromCash('dashboardOserv');
        ...
        fetchData: function (options) {
            ...
            dashCollection = new OservDashboard(options);
        ...
        getCellClass: function (week, self, employee) {
            ...
            var oservs = week.oservs || 0;

            hours = hours + (holidays + oservs) * 8;
        ...
        getCellSize: function (week, oserv) {
            var v = '';
            var w = '';
            var holidays = week.holidays || 0;
            var oservs = week.oservs || 0;
            var oservHours = (holidays + oservs) * 8;
        ...
        changeDateRange: function (dateArray) {
            ...
            filter = this.filter || custom.retriveFromCash('DashOserv.filter') || {};
            filter.date = {
                value: dateArray
            };
            ...
            custom.cacheToApp('DashOserv.filter', this.filter);
        },

        showFilteredPage: function (filter) {
            var dashCollection;
            var thisFilter = this.filter || custom.retriveFromCash('DashOserv.filter');
        ...
        render: function () {
            ...
            var filter = this.filter || custom.retriveFromCash('DashOserv.filter');
            var weeksArr;
            var self = this;
            var statictics;
            var url = '#easyErp/DashBoardOserv';
            ...
            custom.cacheToApp('dashboardOserv', this.dashCollection);

            weeksArr = custom.retriveFromCash('oservDashWeeksArr');
            ...
            if (!this.filterView) {
                if (!App || !App.filtersObject || !App.filtersObject.filtersValues || !App.filtersObject.filtersValues.DashOserv) {
                    custom.getFiltersValues({contentType: 'DashOserv'}, this.renderFilter(this.baseFilter));
                } else {
