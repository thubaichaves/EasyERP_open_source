...
var CapacityHandler = require('./capacity_oserv');
...
var Module = function (event, models) {
    ...
    var OservSchema = mongoose.Schemas.Oserv;
    ...
    function calculate(data, year) {
        ...
        leaveDays = oserv + personal + sick + education;
    ...
    this.getYears = function (req, res, next) {
        var Oserv = models.get(req.session.lastDb, 'Oserv', OservSchema);
        var query;
        var newYear;
        var year;
        /* var lastEl;
    ...
    this.getStatistic = function (req, res, next) {
        ...
        Oserv.aggregate([{
            ...
            oserv = _.find(result, function (item) {
                return item._id === 'V';
            });
            ...
            resObj.oserv = (oserv && oserv.count) || 0;
    ...
    function getOservFilter(req, res, next) {
        var Oserv = models.get(req.session.lastDb, 'Oserv', OservSchema);
    ...
    function getOservByWeek(req, res, next) {
        var Oserv = models.get(req.session.lastDb, 'Oserv', OservSchema);
        var options = req.query;
        ...
        var dateByWeekField = 'oservs.' + dateByWeek;
        ...
        Oserv.aggregate(aggregateQuery,
            function (err, result) {
                if (err) {
                    return next(err);
                }
                result.forEach(function (oserv) {
                    day = oserv.day + 1;
                    month = oserv.month;
                    monthDay = day + 100 * month;
                    if (daysOfMonth[monthDay]) {
                        oservsWeek[daysOfMonth[monthDay]] = oserv.vacArray;
                    }
                });

                res.status(200).send(oservsWeek);
            });
    ...
    this.getForView = function (req, res, next) {
        ...
        if (req.query.week) {
            getOservByWeek(req, res, next);
        } else {
            getOservFilter(req, res, next);
        }
    };
    ...
    this.putchModel = function (req, res, next) {
        ...
        var Oserv = models.get(dbName, 'Oserv', OservSchema);
        var capData = {
            db: dbName
        };
        ...
        data.oservs = calculateWeeks(vacArr, data.month, data.year);

        Oserv.findByIdAndUpdate(id, {$set: data}, {new: true}, function (err, response) {
            if (err) {
                return next(err);
            }

            capacityHandler.oservChanged(capData, next);
            ...
            event.emit('recollectOservDash', {dbName: dbName});
        });
    };
    ...
    this.putchBulk = function (req, res, next) {
        ...
        async.each(body, function (data, cb) {
            ...
            if (data.vacArray) {
                data.oservs = calculateWeeks(data.vacArray, data.month, data.year);
            }

            Oserv.findByIdAndUpdate(id, {$set: data}, {new: true}, function (err, result) {
                if (err) {
                    return cb(err);
                }

                capData.oserv = result.toJSON();
    ...
    this.remove = function (req, res, next) {
        ...
        Oserv.findByIdAndRemove({_id: id}, function (err, oserv) {
            if (err) {
                return next(err);
            }

            res.status(200).send({success: oserv});
            event.emit('setReconcileTimeCard', {
                req     : req,
                month   : oserv.month,
                year    : oserv.year,
                employee: oserv.employee
            });
            event.emit('recollectOservDash', {dbName: dbName});
        });
    };
    ...
    this.create = function (req, res, next) {
        ...
        body.oservs = calculateWeeks(vacArr, body.month, body.year);

        oservKeys = Object.keys(body.oservs);

        oservKeys.forEach(function (key) {
            result += body.oservs[key];
        });
        ...
        oserv = new Oserv(body);

        oserv.save(function (err, oservResult) {
            if (err) {
                return next(err);
            }
            ...
            event.emit('setReconcileTimeCard', {
                req     : req,
                month   : oservResult.month,
                year    : oservResult.year,
                employee: oservResult.employee
            });

            function populateEmployees(cb) {
                Employee.populate(oservResult, {
                    path  : 'employee',
            ...
            function populateDeps(cb) {
                Department.populate(oservResult, {
                    path  : 'department',
                    select: '_id name',
                    lean  : true
                }, cb);
            ...
            async.parallel(parallelTasks, function (err) {
                ...
                res.status(200).send({success: oservResult});
                event.emit('recollectOservDash', {dbName: dbName});
            });
        });
    };
