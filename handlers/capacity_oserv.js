...
var Module = function (models) {
    ...
    function setOservs(model, oserv, db, callback) {
        var vacArrayLength;
        var vacArray;
        var capacityArray;
        var total;
        var month = oserv.month;
        var year = oserv.year;
        var i;
        var dateValue;
        var dayNumber;

        vacArray = oserv.vacArray;
    ...
    function createCapacityOnMonth(db, month, year, callback) {
        ...
        var Oserv = models.get(db, 'Oserv', OservSchema);
        ...
        function saveCapacities(employees, callback) {
            ...
            async.eachLimit(employees, 100, function (employee, cb) {
                ...
                query = Oserv.find(queryObject).lean();
                ...
                query.exec(function (err, result) {
                    ...
                    function getData(seriaCB) {
                        setOservs(modelObject, result, db, function (err, result) {
                            if (err) {
                                return seriaCB(err);
                            }
                    ...
                    if (!result) {
                        modelObject.oserv = null;
                    } else {
                        modelObject.oserv = result._id;
    ...
    this.oservChanged = function (data, next) {
        var oserv = data.oserv;
        var id = oserv.employee._id;
        var month = oserv.month;
        var year = oserv.year;
        var query;
        var db = data.db;
        ...
        query.exec(function (err, result) {
            async.each(result, function (model, eachCB) {
                ...
                setOservs(modelJSON, oserv, db, function (err, result) {
                    if (err) {
                        return eachCB(err);
                    }
